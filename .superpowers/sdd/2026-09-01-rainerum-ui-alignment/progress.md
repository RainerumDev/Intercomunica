...
| 4 — Prenotazioni portal alignment | Complete | Commit `625205f` aligns the authenticated shell and deterministic local login with the official full/compact Rainerum hierarchy, exact institutional tokens, shared page/card/form/dialog/admin geometry, role-aware mobile navigation, and accessible focus treatment. Review fix `5bbb3df` adds the same protected shell to every admin and technician route without nested main landmarks, restores 3:1 form-control boundary contrast, hardens pending cancellation focus/inert containment and trigger restoration, and gives recurring-scope radios native group names/values. Final review evidence: 148/148 database-backed tests, 6/6 clean-seed Playwright tests covering every protected route at the 767/768 breakpoint boundary plus booking cancellation keyboard behavior and computed contrast, typecheck, lint, production build, diff checks, and reverse validation of `review-625205f..5bbb3df.diff`. The 1440×900 calendar was recaptured after a visible non-empty iframe-body signal; 767×844 admin and 768×844 technician captures prove the compact/full mark boundary. Pre-existing `.DS_Store` and `mockups/` remain the only Prenotazioni untracked paths. See `task-4-report.md`. |
