...
- [x] Serve built web from Express in production (static + SPA fallback, smoke-tested) — iter 4
- [ ] E2E smoke with real DB: docker compose + prisma push + seed script + API integration tests (vitest + supertest against test DB)
- [x] Retry with exponential backoff + jitter on Google writes (429/5xx/403-rate) — iter 4
- [x] Master OAuth callback now requires ADMIN session (was: signed state only) — iter 4
- [x] Synclog history table in AdminSettings — iter 4
...
- [x] Cloud Identity API path for member listing without admin role — iter 3 (school can't grant admin role to master). listGroupMembers → cloudidentity memberships.list (direct members only, nested groups skipped; no suspended-status filter — share errors surface per-user in sync). listGroups still Admin SDK (optional dropdown; manual input is the fallback).
